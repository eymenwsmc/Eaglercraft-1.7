...
public class CrashReport {
	...
	private void populateEnvironment() {
		...
		this.theReportCategory.addCrashSectionCallable("Memory", new Callable() {
			...
			public String call() {
				EagRuntime var1 = new EagRuntime();
	...
	public void getSectionsInStringBuilder(StringBuilder builder) {
		if ((this.stacktrace == null || this.stacktrace.length <= 0) && this.crashReportSections.size() > 0) {
			StackTraceElement[] elements = ((CrashReportCategory) this.crashReportSections.get(0)).func_147152_a();
			this.stacktrace = new StackTraceElement[Math.min(1, elements.length)];
			for (int i = 0; i < this.stacktrace.length; ++i) {
				this.stacktrace[i] = elements[i];
			}
		}
	...
	public String getCauseStackTraceOrString() {
		...
		try {
			...
			net.lax1dude.eaglercraft.IOUtils.closeQuietly((Writer) stringwriter);
	...
	private static String getWittyComment() {
		String[] var0 = new String[] { "Who set us up the TNT?", "You eagler!", "You peeing vigg!",
				...
				"But it works on my machine.", "Ur face is making me freaky, can i have 1 backshots?",
				"Awww you feel sad. Please suck my coke", "Dude come on, you look freakyyyyy!",
				"I know you're angry, call an escort!", "Jusr " };
