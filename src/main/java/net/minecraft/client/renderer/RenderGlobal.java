...
public class RenderGlobal implements IWorldAccess {
	...
	public RenderGlobal(Minecraft p_i1249_1_) {
		...
		byte var2 = 34;
		byte var3 = 16;
		this.glRenderListBase = GLAllocation.generateDisplayLists();
		this.displayListEntitiesDirty = false;
		this.displayListEntities = GLAllocation.generateDisplayLists();
		...
		int glList = EaglercraftGPU.glGenLists(1);
		EaglercraftGPU.glNewList(glList, GL11.GL_COMPILE);
		byte var6 = 64;
		int var7 = 256 / var6 + 2;
		float var5 = 16.0F;
		int var8;
		...
		GL11.glEndList();
		this.glSkyList2 = EaglercraftGPU.glGenLists(1);
		EaglercraftGPU.glNewList(this.glSkyList2, GL11.GL_COMPILE);
		var5 = -16.0F;
		var4.startDrawingQuads();
	...
	public void loadRenderers() {
		if (this.theWorld != null) {
				...
				for (int var5 = 0; var5 < this.renderChunksTall; ++var5) {
					for (int var6 = 0; var6 < this.renderChunksDeep; ++var6) {
						this.worldRenderers[(var6 * this.renderChunksTall + var5) * this.renderChunksWide
								+ var4] = new WorldRenderer(this.theWorld, this.tileEntities, var4 * 16, var5 * 16,
										var6 * 16, this.glRenderListBase + var2);

						if (this.occlusionEnabled) {
							this.worldRenderers[(var6 * this.renderChunksTall + var5) * this.renderChunksWide
									+ var4].glOcclusionQuery = this.glOcclusionQueryBase.get(var3);
						}
						...
						this.worldRenderers[(var6 * this.renderChunksTall + var5) * this.renderChunksWide
								+ var4].isVisible = true;
						this.worldRenderers[(var6 * this.renderChunksTall + var5) * this.renderChunksWide
								+ var4].isInFrustum = true;
						this.worldRenderers[(var6 * this.renderChunksTall + var5) * this.renderChunksWide
								+ var4].chunkIndex = var3++;
						this.worldRenderers[(var6 * this.renderChunksTall + var5) * this.renderChunksWide + var4]
								.markDirty();
						this.sortedWorldRenderers[(var6 * this.renderChunksTall + var5) * this.renderChunksWide
								+ var4] = this.worldRenderers[(var6 * this.renderChunksTall + var5)
										* this.renderChunksWide + var4];
						this.worldRenderersToUpdate
								.add(this.worldRenderers[(var6 * this.renderChunksTall + var5) * this.renderChunksWide
										+ var4]);
						var2 += 3;
					}
				}
			...
			if (this.theWorld != null) {
				...
				if (var7 != null) {
					this.markRenderersForNewPosition(MathHelper.floor_double(var7.posX),
							MathHelper.floor_double(var7.posY), MathHelper.floor_double(var7.posZ));
	...
	public void renderEntities(EntityLivingBase p_147589_1_, ICamera p_147589_2_, float p_147589_3_) {
		if (this.renderEntitiesStartupCounter > 0) {
			...
			TileEntityRendererDispatcher.instance.func_147542_a(this.theWorld, this.mc.getTextureManager(),
					this.mc.fontRenderer, this.mc.renderViewEntity, p_147589_3_);
			RenderManager.instance.func_147938_a(this.theWorld, this.mc.getTextureManager(), this.mc.fontRenderer,
			...
			GL11.glPushMatrix();
			GL11.glTranslatef((float) -var11, (float) -var13, (float) -var15);
			...
			for (var18 = 0; var18 < var17.size(); ++var18) {
				...
				boolean var20 = var19.isInRangeToRender3d(var4, var6, var8)
				...
				if (var20
						&& (var19 != this.mc.renderViewEntity || this.mc.gameSettings.thirdPersonView != 0
			...
			for (var18 = 0; var18 < this.tileEntities.size(); ++var18) {
				TileEntityRendererDispatcher.instance.func_147544_a((TileEntity) this.tileEntities.get(var18),
						p_147589_3_);
			}
		...
		for (int var2 = 0; var2 < var1.size(); ++var2) {
			...
			if (RenderManager.instance.getEntityRenderObject(var3).func_147905_a()) {
				this.displayListEntitiesDirty = this.displayListEntitiesDirty
						|| !RenderManager.instance.func_147936_a(var3, 0.0F, true);
			}
		}
		...
		for (int var6 = 0; var6 < this.renderChunksWide; ++var6) {
			...
			for (int var9 = 0; var9 < this.renderChunksDeep; ++var9) {
				...
				for (int var12 = 0; var12 < this.renderChunksTall; ++var12) {
					...
					WorldRenderer var14 = this.worldRenderers[(var9 * this.renderChunksTall + var12)
							* this.renderChunksWide + var6];
	...
	public int sortAndRender(EntityLivingBase p_72719_1_, int p_72719_2_, double p_72719_3_) {
		...
		if (this.prevChunkSortX != p_72719_1_.chunkCoordX || this.prevChunkSortY != p_72719_1_.chunkCoordY
				|| this.prevChunkSortZ != p_72719_1_.chunkCoordZ
				|| var11 * var11 + var13 * var13 + var15 * var15 > 16.0D) {
			...
			this.prevChunkSortZ = p_72719_1_.chunkCoordZ;
			this.markRenderersForNewPosition(MathHelper.floor_double(p_72719_1_.posX),
			...
			do {
				...
				for (int var29 = var41; var29 < var25; ++var29) {
					if (this.sortedWorldRenderers[var29].skipAllRenderPasses()) {
						...
						if (this.sortedWorldRenderers[var29].isInFrustum
								&& !this.sortedWorldRenderers[var29].isWaitingOnOcclusionQuery) {
							float var30 = MathHelper
									.sqrt_float(this.sortedWorldRenderers[var29].distanceToEntitySquared(p_72719_1_));
	...
	private int renderSortedRenderers(int p_72724_1_, int p_72724_2_, int p_72724_3_, double p_72724_4_) {
		...
		for (int var10 = var7; var10 != var8; var10 += var9) {
			...
			if (!this.sortedWorldRenderers[var10].skipRenderPass[p_72724_3_]
					&& this.sortedWorldRenderers[var10].isInFrustum
					&& (!this.occlusionEnabled || this.sortedWorldRenderers[var10].isVisible)) {
		...
		for (var18 = 0; var18 < this.glRenderLists.size(); ++var18) {
			...
			if (var20 < 0) {
				var20 = var17++;
				this.allRenderLists[var20].setupRenderList(var19.posXMinus, var19.posYMinus, var19.posZMinus, var23,
						var13, var15);
			}
	...
	public void renderSky(float p_72714_1_) {
		if (this.mc.theWorld.provider.dimensionId == 1) {
			...
			OpenGlHelper.glBlendFunc(770, 771, 1, 0);
			RenderHelper.disableStandardItemLighting();
			float[] var24 = this.theWorld.provider.calcSunriseSunsetColors(this.theWorld.getCelestialAngle(p_72714_1_),
					p_72714_1_);
			float var9;
			float var10;
			float var11;
			...
			if (var24 != null) {
				...
				GL11.glRotatef(90.0F, 1.0F, 0.0F, 0.0F);
				GL11.glRotatef(
						MathHelper.sin(this.theWorld.getCelestialAngleRadians(p_72714_1_)) < 0.0F ? 180.0F : 0.0F, 0.0F,
						0.0F, 1.0F);
				GL11.glRotatef(90.0F, 0.0F, 0.0F, 1.0F);
				var8 = var24[0];
				var9 = var24[1];
				...
				for (int var27 = 0; var27 <= var26; ++var27) {
					...
					var23.addVertex((double) (var14 * 120.0F), (double) (var15 * 120.0F),
			...
			GL11.glEnable(GL11.GL_BLEND);
			OpenGlHelper.glBlendFunc(770, 1, 1, 0);			GL11.glDisable(GL11.GL_ALPHA_TEST);
			GL11.glPushMatrix();
			...
			GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, 9728 /* GL_NEAREST */);
			GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, 9728 /* GL_NEAREST */);
		...
		if (this.mc.theWorld.provider.isSurfaceWorld()) {
			if (this.mc.gameSettings.fancyGraphics) {
				...
				float var2 = (float) (this.mc.renderViewEntity.lastTickPosY
						+ (this.mc.renderViewEntity.posY - this.mc.renderViewEntity.lastTickPosY)
								* (double) p_72718_1_);
				byte var3 = 32;
				int var4 = 256 / var3;
				...
				double var13 = this.mc.renderViewEntity.prevPosX
						+ (this.mc.renderViewEntity.posX - this.mc.renderViewEntity.prevPosX) * (double) p_72718_1_
						+ var24 * 0.029999999329447746D;
				double var15 = this.mc.renderViewEntity.prevPosZ
						+ (this.mc.renderViewEntity.posZ - this.mc.renderViewEntity.prevPosZ) * (double) p_72718_1_;
	...
	public void renderCloudsFancy(float p_72736_1_) {
		GL11.glDisable(GL11.GL_CULL_FACE);
		float var2 = (float) (this.mc.renderViewEntity.lastTickPosY
				+ (this.mc.renderViewEntity.posY - this.mc.renderViewEntity.lastTickPosY) * (double) p_72736_1_);
		Tessellator var3 = Tessellator.instance;
		float var4 = 12.0F;
		float var5 = 4.0F;
		double var6 = (double) ((float) this.cloudTickCounter + p_72736_1_);
		double var8 = (this.mc.renderViewEntity.prevPosX
				+ (this.mc.renderViewEntity.posX - this.mc.renderViewEntity.prevPosX) * (double) p_72736_1_
				+ var6 * 0.029999999329447746D) / (double) var4;
		double var10 = (this.mc.renderViewEntity.prevPosZ
				+ (this.mc.renderViewEntity.posZ - this.mc.renderViewEntity.prevPosZ) * (double) p_72736_1_)
				/ (double) var4 + 0.33000001311302185D;
				...
				for (int var29 = -var25 + 1; var29 <= var25; ++var29) {
					...
					if (var12 > -var5 - 1.0F) {
						...
						var3.addVertexWithUV((double) (var32 + 0.0F), (double) (var12 + 0.0F),
								(double) (var33 + (float) var24), (double) ((var30 + 0.0F) * var21 + var19),
								(double) ((var31 + (float) var24) * var21 + var20));
						var3.addVertexWithUV((double) (var32 + (float) var24), (double) (var12 + 0.0F),
								(double) (var33 + (float) var24), (double) ((var30 + (float) var24) * var21 + var19),
								(double) ((var31 + (float) var24) * var21 + var20));
						var3.addVertexWithUV((double) (var32 + (float) var24), (double) (var12 + 0.0F),
								(double) (var33 + 0.0F), (double) ((var30 + (float) var24) * var21 + var19),
								(double) ((var31 + 0.0F) * var21 + var20));
						var3.addVertexWithUV((double) (var32 + 0.0F), (double) (var12 + 0.0F), (double) (var33 + 0.0F),
								(double) ((var30 + 0.0F) * var21 + var19), (double) ((var31 + 0.0F) * var21 + var20));
					}

					if (var12 <= var5 + 1.0F) {
						var3.setColorRGBA_F(var16, var17, var18, 0.8F);
						var3.setNormal(0.0F, 1.0F, 0.0F);
						var3.addVertexWithUV((double) (var32 + 0.0F), (double) (var12 + var5 - var26),
								(double) (var33 + (float) var24), (double) ((var30 + 0.0F) * var21 + var19),
								(double) ((var31 + (float) var24) * var21 + var20));
						var3.addVertexWithUV((double) (var32 + (float) var24), (double) (var12 + var5 - var26),
								(double) (var33 + (float) var24), (double) ((var30 + (float) var24) * var21 + var19),
								(double) ((var31 + (float) var24) * var21 + var20));
						var3.addVertexWithUV((double) (var32 + (float) var24), (double) (var12 + var5 - var26),
								(double) (var33 + 0.0F), (double) ((var30 + (float) var24) * var21 + var19),
					...
					if (var28 > -1) {
						...
						for (var34 = 0; var34 < var24; ++var34) {
							...
							var3.addVertexWithUV((double) (var32 + (float) var34 + 0.0F), (double) (var12 + var5),
									...
									(double) ((var31 + (float) var24) * var21 + var20));
							var3.addVertexWithUV((double) (var32 + (float) var34 + 0.0F), (double) (var12 + var5),
									(double) (var33 + 0.0F), (double) ((var30 + (float) var34 + 0.5F) * var21 + var19),
									(double) ((var31 + 0.0F) * var21 + var20));
							var3.addVertexWithUV((double) (var32 + (float) var34 + 0.0F), (double) (var12 + 0.0F),
									(double) (var33 + 0.0F), (double) ((var30 + (float) var34 + 0.5F) * var21 + var19),
									(double) ((var31 + 0.0F) * var21 + var20));
					...
					if (var28 <= 1) {
						...
						for (var34 = 0; var34 < var24; ++var34) {
							var3.addVertexWithUV((double) (var32 + (float) var34 + 1.0F - var26),
									(double) (var12 + 0.0F), (double) (var33 + (float) var24),
									(double) ((var30 + (float) var34 + 0.5F) * var21 + var19),
									(double) ((var31 + (float) var24) * var21 + var20));
							var3.addVertexWithUV((double) (var32 + (float) var34 + 1.0F - var26),
									(double) (var12 + var5), (double) (var33 + (float) var24),
									(double) ((var30 + (float) var34 + 0.5F) * var21 + var19),
									(double) ((var31 + (float) var24) * var21 + var20));
							var3.addVertexWithUV((double) (var32 + (float) var34 + 1.0F - var26),
					...
					if (var29 > -1) {
						...
						for (var34 = 0; var34 < var24; ++var34) {
							var3.addVertexWithUV((double) (var32 + 0.0F), (double) (var12 + var5),
									(double) (var33 + (float) var34 + 0.0F), (double) ((var30 + 0.0F) * var21 + var19),
									(double) ((var31 + (float) var34 + 0.5F) * var21 + var20));
							var3.addVertexWithUV((double) (var32 + (float) var24), (double) (var12 + var5),
									(double) (var33 + (float) var34 + 0.0F),
									(double) ((var30 + (float) var24) * var21 + var19),
									(double) ((var31 + (float) var34 + 0.5F) * var21 + var20));
							var3.addVertexWithUV((double) (var32 + (float) var24), (double) (var12 + 0.0F),
									(double) (var33 + (float) var34 + 0.0F),
									(double) ((var30 + (float) var24) * var21 + var19),
					...
					if (var29 <= 1) {
						...
						for (var34 = 0; var34 < var24; ++var34) {
							var3.addVertexWithUV((double) (var32 + 0.0F), (double) (var12 + var5),
									(double) (var33 + (float) var34 + 1.0F - var26),
			...
			if (var10 != null) {
				if (!p_72716_2_) {
					if (var10.distanceToEntitySquared(p_72716_1_) > 272.0F) {
						for (var11 = 0; var11 < var3
								&& (var5[var11] == null || var4.compare(var5[var11], var10) <= 0); ++var11) {
							;
						}
	...
	public void drawSelectionBox(EntityPlayer p_72731_1_, MovingObjectPosition p_72731_2_, int p_72731_3_,
		...
		if (p_72731_3_ == 0 && p_72731_2_.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
			...
			if (var6.getMaterial() != Material.air) {
				...
				double var7 = p_72731_1_.lastTickPosX
						+ (p_72731_1_.posX - p_72731_1_.lastTickPosX) * (double) p_72731_4_;
				double var9 = p_72731_1_.lastTickPosY
						+ (p_72731_1_.posY - p_72731_1_.lastTickPosY) * (double) p_72731_4_;
				double var11 = p_72731_1_.lastTickPosZ
						+ (p_72731_1_.posZ - p_72731_1_.lastTickPosZ) * (double) p_72731_4_;
				drawOutlinedBoundingBox(var6
						.getSelectedBoundingBoxFromPool(this.theWorld, p_72731_2_.blockX, p_72731_2_.blockY,
								p_72731_2_.blockZ)
						.expand((double) var5, (double) var5, (double) var5).getOffsetBoundingBox(-var7, -var9, -var11),
						-1);
			}
	...
	public void clipRenderersByFrustum(ICamera p_72729_1_, float p_72729_2_) {
		for (int var3 = 0; var3 < this.worldRenderers.length; ++var3) {
			if (!this.worldRenderers[var3].skipAllRenderPasses()
					&& (!this.worldRenderers[var3].isInFrustum || (var3 + this.frustumCheckOffset & 15) == 0)) {
	...
	public void playRecord(String p_72702_1_, int p_72702_2_, int p_72702_3_, int p_72702_4_) {
		...
		if (p_72702_1_ != null) {
			...
			PositionedSoundRecord var8 = PositionedSoundRecord.createPositionedSoundRecord(
					new ResourceLocation(p_72702_1_), (float) p_72702_2_, (float) p_72702_3_, (float) p_72702_4_);
	...
	public EntityFX doSpawnParticle(String p_72726_1_, double p_72726_2_, double p_72726_4_, double p_72726_6_,
		...
		if (this.mc != null && this.mc.renderViewEntity != null && this.mc.effectRenderer != null) {
			...
			if (p_72726_1_.equals("hugeexplosion")) {
				this.mc.effectRenderer.addEffect(var21 = new EntityHugeExplodeFX(this.theWorld, p_72726_2_, p_72726_4_,
						p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_));
			} else if (p_72726_1_.equals("largeexplode")) {
				this.mc.effectRenderer.addEffect(var21 = new EntityLargeExplodeFX(this.renderEngine, this.theWorld,
						p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_));
			} else if (p_72726_1_.equals("fireworksSpark")) {
				this.mc.effectRenderer.addEffect(var21 = new EntityFireworkSparkFX(this.theWorld, p_72726_2_,
						p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_, this.mc.effectRenderer));
			}
			...
			if (var21 != null) {
				...
				if (var15 * var15 + var17 * var17 + var19 * var19 > var22 * var22) {
					...
					if (p_72726_1_.equals("bubble")) {
						var21 = new EntityBubbleFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("suspended")) {
						var21 = new EntitySuspendFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("depthsuspend")) {
						var21 = new EntityAuraFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("townaura")) {
						var21 = new EntityAuraFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("crit")) {
						var21 = new EntityCritFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
						...
						var21 = new EntitySmokeFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("mobSpell")) {
						var21 = new EntitySpellParticleFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, 0.0D, 0.0D,
								0.0D);
						...
						var21 = new EntitySpellParticleFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
						...
						var21 = new EntityNoteFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("portal")) {
						var21 = new EntityPortalFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("enchantmenttable")) {
						var21 = new EntityEnchantmentTableParticleFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_,
								p_72726_8_, p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("explode")) {
						var21 = new EntityExplodeFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("flame")) {
						var21 = new EntityFlameFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
						...
						var21 = new EntityFootStepFX(this.renderEngine, this.theWorld, p_72726_2_, p_72726_4_,
								p_72726_6_);
					} else if (p_72726_1_.equals("splash")) {
						var21 = new EntitySplashFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("wake")) {
						var21 = new EntityFishWakeFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("largesmoke")) {
						var21 = new EntitySmokeFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_, 2.5F);
					} else if (p_72726_1_.equals("cloud")) {
						var21 = new EntityCloudFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("reddust")) {
						var21 = new EntityReddustFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_,
								(float) p_72726_8_, (float) p_72726_10_, (float) p_72726_12_);
						...
						var21 = new EntityDropParticleFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_,
								Material.water);
					} else if (p_72726_1_.equals("dripLava")) {
						var21 = new EntityDropParticleFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_,
								Material.lava);
					} else if (p_72726_1_.equals("snowshovel")) {
						var21 = new EntitySnowShovelFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("slime")) {
						var21 = new EntityBreakingFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_,
								Items.slime_ball);
					} else if (p_72726_1_.equals("heart")) {
						var21 = new EntityHeartFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
					} else if (p_72726_1_.equals("angryVillager")) {
						var21 = new EntityHeartFX(this.theWorld, p_72726_2_, p_72726_4_ + 0.5D, p_72726_6_, p_72726_8_,
								p_72726_10_, p_72726_12_);
						...
						if (p_72726_1_.startsWith("iconcrack_")) {
							...
							if (var27.length > 2) {
								...
								var21 = new EntityBreakingFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_,
										p_72726_8_, p_72726_10_, p_72726_12_, Item.getItemById(var25), var26);
							} else {
								var21 = new EntityBreakingFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_,
										p_72726_8_, p_72726_10_, p_72726_12_, Item.getItemById(var25), 0);
							}
						} else {
							Block var28;
							...
							if (p_72726_1_.startsWith("blockcrack_")) {
								...
								var26 = Integer.parseInt(var27[2]);
								var21 = (new EntityDiggingFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_,
										p_72726_8_, p_72726_10_, p_72726_12_, var28, var26)).applyRenderColor(var26);
							} else if (p_72726_1_.startsWith("blockdust_")) {
								var27 = p_72726_1_.split("_", 3);
								var28 = Block.getBlockById(Integer.parseInt(var27[1]));
								var26 = Integer.parseInt(var27[2]);
								var21 = (new EntityBlockDustFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_,
										p_72726_8_, p_72726_10_, p_72726_12_, var28, var26)).applyRenderColor(var26);
							}
						}
					}
	...
	public void playAuxSFX(EntityPlayer p_72706_1_, int p_72706_2_, int p_72706_3_, int p_72706_4_, int p_72706_5_,
		...
		switch (p_72706_2_) {
		case 1000:
			this.theWorld.playSound((double) p_72706_3_, (double) p_72706_4_, (double) p_72706_5_, "random.click", 1.0F,
					1.0F, false);
			break;
		...
		case 1002:
			this.theWorld.playSound((double) p_72706_3_, (double) p_72706_4_, (double) p_72706_5_, "random.bow", 1.0F,
					1.2F, false);
			break;

		case 1003:
			if (Math.random() < 0.5D) {
				this.theWorld.playSound((double) p_72706_3_ + 0.5D, (double) p_72706_4_ + 0.5D,
						(double) p_72706_5_ + 0.5D, "random.door_open", 1.0F,
						this.theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			} else {
				this.theWorld.playSound((double) p_72706_3_ + 0.5D, (double) p_72706_4_ + 0.5D,
						(double) p_72706_5_ + 0.5D, "random.door_close", 1.0F,
						this.theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			}
		...
		case 1004:
			this.theWorld.playSound((double) ((float) p_72706_3_ + 0.5F), (double) ((float) p_72706_4_ + 0.5F),
					(double) ((float) p_72706_5_ + 0.5F), "random.fizz", 0.5F,
					2.6F + (var7.nextFloat() - var7.nextFloat()) * 0.8F, false);
			break;

		case 1005:
			if (Item.getItemById(p_72706_6_) instanceof ItemRecord) {
				this.theWorld.playRecord("records." + ((ItemRecord) Item.getItemById(p_72706_6_)).field_150929_a,
						p_72706_3_, p_72706_4_, p_72706_5_);
			} else {
		...
		case 1007:
			this.theWorld.playSound((double) p_72706_3_ + 0.5D, (double) p_72706_4_ + 0.5D, (double) p_72706_5_ + 0.5D,
		...
		case 1020:
			this.theWorld.playSound((double) ((float) p_72706_3_ + 0.5F), (double) ((float) p_72706_4_ + 0.5F),
					(double) ((float) p_72706_5_ + 0.5F), "random.anvil_break", 1.0F,
					this.theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			break;

		case 1021:
			this.theWorld.playSound((double) ((float) p_72706_3_ + 0.5F), (double) ((float) p_72706_4_ + 0.5F),
					(double) ((float) p_72706_5_ + 0.5F), "random.anvil_use", 1.0F,
					this.theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			break;

		case 1022:
			this.theWorld.playSound((double) ((float) p_72706_3_ + 0.5F), (double) ((float) p_72706_4_ + 0.5F),
					(double) ((float) p_72706_5_ + 0.5F), "random.anvil_land", 0.3F,
					this.theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
			...
			if (var8.getMaterial() != Material.air) {
				this.mc.getSoundHandler()
						.playSound(new PositionedSoundRecord(new ResourceLocation(var8.stepSound.func_150495_a()),
								(var8.stepSound.func_150497_c() + 1.0F) / 2.0F, var8.stepSound.func_150494_d() * 0.8F,
								(float) p_72706_3_ + 0.5F, (float) p_72706_4_ + 0.5F, (float) p_72706_5_ + 0.5F));
			}
		...
		case 2002:
			...
			for (var40 = 0; var40 < 100; ++var40) {
				...
				EntityFX var42 = this.doSpawnParticle(var20, var9 + var26 * 0.1D, var11 + 0.3D, var13 + var30 * 0.1D,
						var26, var28, var30);
		...
		case 2003:
			...
			for (double var36 = 0.0D; var36 < (Math.PI * 2D); var36 += 0.15707963267948966D) {
				this.spawnParticle("portal", var9 + Math.cos(var36) * 5.0D, var11 - 0.4D,
						var13 + Math.sin(var36) * 5.0D, Math.cos(var36) * -5.0D, 0.0D, Math.sin(var36) * -5.0D);
				this.spawnParticle("portal", var9 + Math.cos(var36) * 5.0D, var11 - 0.4D,
						var13 + Math.sin(var36) * 5.0D, Math.cos(var36) * -7.0D, 0.0D, Math.sin(var36) * -7.0D);
			}
				...
				for (int var24 = 0; var24 < var23; ++var24) {
					...
					this.theWorld.spawnParticle(
							"blockdust_" + Block.getIdFromBlock(var8) + "_"
									+ this.theWorld.getBlockMetadata(p_72706_3_, p_72706_4_, p_72706_5_),
							(double) ((float) p_72706_3_ + 0.5F), (double) ((float) p_72706_4_ + 1.0F),
							(double) ((float) p_72706_5_ + 0.5F), var30, var28, var32);
				}
			}
		}
	}
