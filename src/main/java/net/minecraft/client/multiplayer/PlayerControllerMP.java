...
public class PlayerControllerMP {
	...
	public void updateController() {
		this.syncCurrentPlayItem();
		NetHandlerPlayClient handler = this.mc.thePlayer != null ? this.mc.thePlayer.sendQueue : null;
		if (handler == null)
			return;		if (handler.getNetworkManager().isChannelOpen()) {
			try {
				handler.getNetworkManager().processReceivedPackets();
				...
				handler.getNetworkManager().closeChannel(new ChatComponentText("Exception thrown: " + ex.toString()));
			}

		} else {
			handler.getNetworkManager().checkDisconnected();
		}
	...
	public boolean onPlayerRightClick(EntityPlayer p_78760_1_, World p_78760_2_, ItemStack p_78760_3_, int p_78760_4_,
		...
		logger.info(
				"[CLIENT] Sending C08PacketPlayerBlockPlacement x={}, y={}, z={}, face={}, item={}, hit=({}, {}, {})",
				p_78760_4_, p_78760_5_, p_78760_6_, p_78760_7_, p_78760_1_.inventory.getCurrentItem(), var9, var10,
				var11);
	...
	public boolean sendUseItem(EntityPlayer p_78769_1_, World p_78769_2_, ItemStack p_78769_3_) {
		this.syncCurrentPlayItem();
		logger.info("[CLIENT] Sending C08PacketPlayerBlockPlacement (use item) x=-1,y=-1,z=-1, face=255, item={}",
				p_78769_1_.inventory.getCurrentItem());
	...
	public ItemStack windowClick(int windowId, int slotId, int mouseButton, int modifier, EntityPlayer player) {

		ItemStack preItem = null;
		try {
			if (player != null && player.openContainer != null && slotId >= 0
					&& slotId < player.openContainer.inventorySlots.size()) {
				preItem = player.openContainer.getSlot(slotId).getStack();
			}
		} catch (Throwable t) {
		...
		try {
			if (mouseButton == 0) {
				...
				String title = null;
				if (isChestGui && player != null && player.openContainer instanceof ContainerChest) {
					IInventory lower = ((ContainerChest) player.openContainer).getLowerChestInventory();
					title = lower != null ? lower.getInventoryName() : null;
				}
				String normTitle = title != null ? title.replace("§", "").toLowerCase().trim() : null;
				String display = preItem != null ? preItem.getDisplayName() : null;
				String normItem = display != null ? display.replace("§", "").toLowerCase().trim() : null;
				if (isChestGui) {
					String target = null;
					if (normItem != null) {
						if (normItem.contains("oneblock"))
							target = "oneblock";
						else if (normItem.contains("skyblock"))
							target = "skyblock";
						else if (normItem.contains("survival"))
							target = "survival";
						else if (normItem.contains("lifesteal"))
							target = "lifesteal";
	...
	public void sendSlotPacket(ItemStack p_78761_1_, int p_78761_2_) {
		if (this.currentGameType.isCreative()) {
			logger.info("[CLIENT] Sending C10PacketCreativeInventoryAction slot={}, item={}", p_78761_2_, p_78761_1_);
	...
	public void sendPacketDropItem(ItemStack p_78752_1_) {
		if (this.currentGameType.isCreative() && p_78752_1_ != null) {
			logger.info("[CLIENT] Sending C10PacketCreativeInventoryAction (drop) slot=-1, item={}", p_78752_1_);
