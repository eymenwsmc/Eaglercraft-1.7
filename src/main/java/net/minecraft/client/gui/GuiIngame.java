...
public class GuiIngame extends Gui {
			...
			if (tpsAge < 20000l) {
				...
				for (int j = 0, m = strs.size(); j < m; ++j) {
					...
					if (!StringUtils.isNullOrEmpty(str)) {
						l = (int) (mc.fontRenderer.getStringWidth(str) * (!first ? 0.5f : 1.0f));
						GlStateManager.pushMatrix();
						GlStateManager.translate(parScaledResolution.getScaledWidth() - 2 - l, i + 2, 0.0f);
						if (!first) {
							GlStateManager.scale(0.5f, 0.5f, 0.5f);
						}
						mc.fontRenderer.drawStringWithShadow(str, 0, 0, color);
						GlStateManager.popMatrix();
						if (color == 0xFFFFFF) {
							color = 14737632;
						}
					}
					i += (int) (mc.fontRenderer.FONT_HEIGHT * (!first ? 0.5f : 1.0f));
					first = false;
				}
			}
		}
	...
	public void renderGameOverlay(float p_73830_1_, boolean p_73830_2_, int p_73830_3_, int p_73830_4_) {
		...
		if (this.mc.gameSettings.showDebugInfo) {
			...
			long var36 = EagRuntime.maxMemory();
			...
			this.drawString(var8,
					HString.format("x: %.5f (%d) // c: %d (%d)", new Object[] { Double.valueOf(this.mc.thePlayer.posX),
							Integer.valueOf(var22), Integer.valueOf(var22 >> 4), Integer.valueOf(var22 & 15) }),
					2, 64, 14737632);
			this.drawString(var8, HString.format("y: %.3f (feet pos, %.3f eyes pos)", new Object[] {
					Double.valueOf(this.mc.thePlayer.boundingBox.minY), Double.valueOf(this.mc.thePlayer.posY) }), 2,
					72, 14737632);
			this.drawString(var8,
					HString.format("z: %.5f (%d) // c: %d (%d)", new Object[] { Double.valueOf(this.mc.thePlayer.posZ),
			...
			this.drawString(var8,
					HString.format("ws: %.3f, fs: %.3f, g: %b, fl: %d",
